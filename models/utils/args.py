...
def parse_args():
    ...
    epoch_capability_group.add_argument('--minibatch',
                                        ...
                                        type=float,
                                        default=None)
    epoch_capability_group.add_argument('-E',
...
def check_args(args):
    ...
    if (args.client_algorithm in SA_MINIMIZERS) and (args.rho_l is None or args.eta is None):
        print("Specificy values for rho_l, eta to run with a client-side sharpness-aware opt")
        exit(-1)
    
    if args.algorithm == "fedgloss" and (args.beta is None or args.rho is None):
        print("Specify beta and rho to run fedgloss")
        exit(-1)
