    ...
    **kwargs:
    ...
    if mode == "power_iter":
        eigenvals, eigenvecs = deflated_power_iteration(
            hvp_operator, num_eigenthings, power_iter_steps=power_iter_steps, use_gpu=use_gpu, fp16=fp16, **kwargs
        )
    else:
        raise ValueError("Unsupported mode %s (must be power_iter)" % mode)
