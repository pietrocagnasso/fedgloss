...
from .operator import Operator, LambdaOperator
...
def deflated_power_iteration(
    ...
    for i in range(num_eigenthings):
        ...
        def _new_op_fn(x, op=current_op, val=eigenval, vec=eigenvec):
            return maybe_fp16(op.apply(x), fp16) - _deflate(x, val, vec)
...
def power_iteration(
    ...
    prev_lambda = 0.0
    prev_vec = maybe_fp16(torch.randn_like(vec), fp16)
    for i in range(steps):
        prev_vec = vec / (torch.norm(vec) + 1e-6)
        new_vec = maybe_fp16(operator.apply(vec), fp16) - momentum * prev_vec
